...
#define ALLOC(allocator, size)                                                 \
  (allocator)->alloc((Allocator *)(allocator), size)
...
#define da_reserve(da, size)                                                   \
  do {                                                                         \
    ...
    if ((size) > (da)->cap) {                                                  \
      (da)->cap = size;                                                        \
      (da)->data =                                                             \
          REALLOC((da)->alloc, (da)->data, sizeof(*(da)->data) * (da)->cap);   \
...
#define da_foreach(it, da)                                                     \
  for (typeof(*(da)->data) *it = (da)->data; it < (da)->data + (da)->len; ++it)
